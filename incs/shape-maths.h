...
typedef struct s_cylinder_hit
{
	...
	float	a;              // Quadratic term coefficient (related to ray.direction and cylinder axis)
	float	h;              // Scalar value for the ray's parameter t along the projected direction
	float	c;              // Constant term in the quadratic equation
	float	discriminant;   // Discriminant of the quadratic equation (used to determine intersection)
